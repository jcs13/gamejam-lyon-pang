...
var DEBUG = false;
var game = new Phaser.Game(700, 500, Phaser.AUTO, 'pang-example', {
	preload : preload,
	create : create,
	update : update,
	render : render
...
function preload() {
	...
	game.load.image('lace', 'assets/lace.png');
...
function create() {
	...
	// Set the world (global) gravity
	eventText = game.add.text(300, 30, 'Next Event', {
		font : '14px "Georgia"',
		fill : '#fff',
		strokeThickness : 1,
		align : 'center',
	...
	ball = balls.create(400, 200, 'ball', 1);

	lace = laces.create(0, game.world.height - 880, 'lace', 1);
	lace2 = laces.create(0, game.world.height - 880, 'lace', 1);
	...
	timerEvents = game.time.events.loop(Phaser.Timer.SECOND * 20, nextball,
			this);

}
function nextball() {

	ball2 = balls.create(400, 10, 'ball' + Math.round(scake, 1), 1);

	gravity = (-1) * gravity;
	ball2.body.collideWorldBounds = true;
	if (gravity < 0) {
		ball2.body.velocity.setTo(110, 700);
	} else {
...
function update() {
	...
	game.physics.arcade.collide(platforms, knocker);
	game.physics.arcade.overlap(laces, platforms, killit, null, this);
	game.physics.arcade.overlap(knocker, balls, reaset, null, this);
	game.physics.arcade.overlap(laces, balls, dostuff, null, this);
...
function dostuff(lace, ball) {
	...
	if (ball.scale.x < 0.25) {
		...
		for (var i = 0; i <= 2; i++) {
			...
			if (i == 1) {
				ball = balls.create((lacex), y, 'ball'
						+ Math.round(scake - 0.2, 1));
				ball.body.velocity.setTo(20 + velox, -20 - veloy / 2);

			} else if (i == 2) {
				ball = balls.create(lacex, y, 'ball'
						+ Math.round(scake - 0.2, 1));
				ball.body.velocity.setTo(-15 - velox, 20 - veloy / 2);
			...
			ball.scale.setTo(scalex / 2, scaley / 2);
			ball.body.gravity.setTo(0, gravity);
			// This sets the gravity the sprite responds to in the world, as a
			// point
			// Here we leave x=0 and set y=80 to simulate falling
			ball.body.collideWorldBounds = true;
